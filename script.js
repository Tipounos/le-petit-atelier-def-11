...
if (input) {
  ...
  input.value = displayValue(range.value);  
   ...
   // ── TABS ──
    document.querySelectorAll('.galerie-tab').forEach(btn => {
      btn.addEventListener('click', () => {
        document.querySelectorAll('.galerie-tab').forEach(b => b.classList.remove('active'));
        document.querySelectorAll('.galerie-panel').forEach(p => p.classList.remove('active'));
        btn.classList.add('active');
        document.getElementById(btn.dataset.target).classList.add('active');
        window.scrollTo({ top: document.querySelector('.galerie-tabs').offsetTop - 72, behavior: 'smooth' });
        console.log("clicukuypuoupyyptpyiyyoypouygohitohjhkgpjhymo");
  
      });
    });
 
    // ── ACCORDION sections ──
    document.querySelectorAll('.galerie-section__header').forEach(header => {
      header.addEventListener('click', () => {
        header.closest('.galerie-section').classList.toggle('open');
      });
    });
    ...
    document.querySelectorAll('.galerie-photo').forEach(photo => {
      photo.addEventListener('click', () => {
        lbImg.src = photo.dataset.src;
        lbImg.alt = photo.dataset.caption || '';
        lbCap.textContent = photo.dataset.caption || '';
